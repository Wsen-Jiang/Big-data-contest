...
class Conv1DResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=None, dilation=1, use_batchnorm=True, activation=nn.ReLU):
        """
        初始化 Conv1D 残差块。
...
class ResBlock_Classifier(nn.Module):
    def __init__(self, channels=2, num_classes=10, dropout=0.5):
        super(ResBlock_Classifier, self).__init__()
        ...
        self.fc = nn.Linear(256, num_classes)  # 最终分类层
    ...
    def forward(self, x): # (bz,2,512)
        ...
        x = torch.cat((x1,x),dim=1) # (bz,16,512)
        x = self.res1(x)
        x = self.pool(x) # (bz,16,128)

        x2 = x
        x = F.relu(self.bn2(self.conv2(x)))
        x = torch.cat((x2,x),dim=1) # (bz,48,512)
        x = self.res2(x)
        x = self.pool(x) # (bz,48,32)        

        x3 = x
        x = F.relu(self.bn3(self.conv3(x)))
        x = torch.cat((x3,x),dim=1) # (bz,48,512)
        x = self.res3(x)
        x = self.pool(x) # (bz,48,32)      
